...
SELECT*FROM user1;
--영어는 단어 1개가 1byte,한국어는 단어 1개가 3byte
...
--CRATE TABLE data varchar2(5)에서  저장할 수 있는 공간이 5개인데 '한국'은 6바이트로 범위를 벗어나기 때문에 오류가 생겻음 
INSERT INTO user1(data)
VALUES('한국');

--특정범위의 문자를 추출해주는 함수이다.
--subst('문자',시작위치,갯수) /*java: subsctring(start, end)*/
...
-- 주어진 문자열에서 왼쪽의 공백제거
SELECT '     korea', length('     korea'), ltrim('     korea'), length('     korea')
FROM dual;
...
SELECT to_char(sysdate, 'yyyy-mm-dd hh:mi:ss dy') --위에는 (화요일), dy는 (화) 만 출력
FROM dual;

SELECT to_char(sysdate, 'yyyy-mm-dd hh24:mi:ss day')  /* 24시간  */  -- 시간 추가
FROM dual;
...
문자->숫자
==================================*/
SELECT '253' || '12' --문자 + 문자 연결
FROM dual;  --25312

SELECT concat('253','12') -- 문자 + 문자 연결
FROM dual;  --25312

SELECT '253' + '12' -- 숫자 + 숫자 연산
FROM dual;   --265

SELECT to_number('253') + to_number('12') --문자를 숫자로 바꾼다.(연산)
FROM dual;   --265
...
/*====================================================================
일반함수
 컬럼 값이랑 대체값 데이터 타입이 동일해야한다. ex: (날짜 - 날짜)
nvl(컬럼, 대체값) : 첫번째 인자값이 null(0)이면 0(대체값)으로 대체해서 출력한다.
...
--count(컬럼) : 개수 / count(컬럼명)을 넣어주면 null값이 아닌 레코드 갯수만 출력
SELECT count(salary)
FROM employees;
...
SELECT count(commission_pct) /* commission_pct값이 null이 아닐때 레코드 수를 리턴*/
FROM employees;
...
SELECT department_id, count(*)
FROM employees
WHERE department_id<=50 AND department_id IS NOT NULL -- 각 레코드에 대한 조건
GROUP BY department_id
ORDER BY department_id;
...
FROM employees
GROUP BY department_id
HAVING department_id<=50 AND department_id IS NOT NULL -- 호명한 그룹에 대한 조건
ORDER BY department_id;
...
FROM employees
GROUP BY department_id
HAVING department_id<=50 AND department_id IS NOT NULL AND count(*)<=5 -- 호명한 그룹에 대한 조건
ORDER BY department_id;
...
SELECT department_id, min(salary), max(salary),min(salary)-max(salary)
FROM employees
GROUP BY department_id 
...
--manager_id 는 숫자가 아니여서 null값으로 바꾸기위해 to_char
...
SELECT avg(salary) 
FROM employees
WHERE job_id = 'IT_PROG'; -- 'IT_PROG'하나만 구하니까 셀렉트절 직종 언급 x

SELECT job_id,avg(salary) 
FROM employees -- 'IT_PROG'하나만 구하니까 셀렉트절 직종 언급 x
GROUP BY job_id
...
SELECT decode(to_char(hire_date,'yyyy'),'2001',1,0) AS "2001"--  to_char(hire_date이 값이 2001년이 이면 1 아니면 0
        decode(to_char(hire_date,'yyyy'),'2002',1,0) AS "2002"
        decode(to_char(hire_date,'yyyy'),'2003',1,0) AS "2003"
FROM employees;
...
FROM employees
GROUP BY department_id -- 그룹화-> 해빙절에서 진행되기 때문에 해빙절에서 따로 디파트먼트안쓰고 카운터로 진행했다.
HAVING count(*)>=10
ORDER BY department_id;
